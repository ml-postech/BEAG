...
class Reacher3DEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def get_image(self, goal=None, subgoal=None, waypoint=None):
        if goal is not None:
            ...
            self.sim.data.site_xpos[self.model.site_name2id('goal_point:box')] = np.array([goal[0], goal[1], goal[2]])
        if subgoal is not None:
            ...
            self.sim.data.site_xpos[self.model.site_name2id('subgoal_point:box')] = np.array([subgoal[0], subgoal[1], subgoal[2]])
        if waypoint is not None:
            ...
            self.sim.data.site_xpos[self.model.site_name2id('way_point:box')] = np.array([waypoint[0], waypoint[1], waypoint[2]])
